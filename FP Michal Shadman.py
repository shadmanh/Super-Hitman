    ...
    def move(self,player,collide):
        ...
        if blockedPath(collide,self.player,self)==True:
            ...
            if self.dist>350: #If they are a decent way away from the player, they B-line towards him
                ...
                moveX=(self.player.x-self.x)*self.speed/dist #distance along the X-axis they move
                moveY=(self.player.y-self.y)*self.speed/dist #distance along the Y-axis they move
                self.ang=degrees(atan2(-moveY,moveX))
                ...
                dist=max(self.dist,1)
                moveX=(self.player.x-self.x)*self.speed/dist
                moveY=(self.player.y-self.y)*self.speed/dist
                self.ang=degrees(atan2(-moveY,moveX))
                if collide.get_at((int(self.x-moveX),int(self.y-moveY)))[:3]!=(0,0,0):
                    self.x-=moveX
                    self.y-=moveY
...
class Wave:
    "The class of the wave number of the enemy AIs"
    def __init__(self):
        self.waveNum=0
        self.enemiesLeft=0
        self.maxEnemies=2 #the maximum number of AIs that can be spawned at any given time

    def startGame(self):
        "Start the game by making enemies spawn"
        self.waveNum=1
        self.enemiesLeft=self.waveNum*5

    def newWave(self,enemyHealth):
        "If all the enemies are killed in a wave, the game gets a little bit harder"
        if self.waveNum%3==0 and self.maxEnemies<5: #every 3 waves there can be 1 more enemy spawned at all times with a
            self.maxEnemies+=1 #cap at 5 because on slower computers thats the most that there can be with small amounts of lag
        self.waveNum+=1 #updates wave number
        self.enemiesLeft=self.waveNum*5 #updates the number of enemies in the wave
        enemyHealth+=1 #increases enemy health
        return enemyHealth
...
def erase(player,peacefulAIlist,bulletList,coinList,deadList,damageTip,
          secondary,secWeap,knifeDamage,droppedClipList,weaponsAI,
          homingBulletList,damageRadius,grenadeDamage,grenadex,grenadey,
          damageTime,collide,horizCollide,wave,score,tScore,multiplier,knifeScore,bulletScore,homingScore,
          reflectScore,grenadeScore,killReduc,reflectingBulletList,
    ...
    for ai in peacefulAIlist:
        ...
        if ai.health<1:
            tScore += score * multiplier #combo ends if a civilian dies
            multiplier = 1
            score = 0
            pList.append(ai)
    ...
    for bullet in bulletList: #checks for collisions on the map
        if collide.get_at((int(bullet.tx),int(bullet.ty)))==(0,0,0):
            ...
            invinCounter = invinSec * 60 #player is invincible for invinSec(onds)
            tScore += score * multiplier #combo ends if player gets damaged
            multiplier = 1
            score = 0
    invinCounter -= 1
...
def gunRun(mb,w,magazine,shot,delay,weaponsList,weapon,player,mx,my,cx,cy,cur_x,cur_y,bList,bulletSpeed,bulletLength,secondaryFire,
        ...
        if player.ammo > 0:
            if delay % weapon[2] == 0 and player.reloading==False:
                ...
                if reflectingBullets:
                    ...
                    if len(AIList)>0: #if there are enemies to home towards
                        for i in range(weapon[1]):
                            ...
                            player.ammo-=1
                        player.ammoInClip=player.ammo%player.clipSize
                    else : #if all the enemies are killed in the wave, the bullets just move like (and literaly are) regular bullets
                        for i in range(weapon[1]):
    ...
    for i in range(len(bList)): #moves all the regular bullets
        bList[i].move(bulletSpeed,bulletLength)

    hList=[]
    for b in homingBulletList:
        if len(AIList)>0: #if it can choose a target
            b.selectTarget(AIList)
            b.home(homingBulletSpeed,bulletLength)
        else : #if there are no enemies to be shot
            bList.append(Bullet(b.ex,b.ey,b.tx,b.ty,b.ang,b.damage))
            hList.append(b) #bullets are made into regular bullets
    length=len(homingBulletList) #and are removed from the homing bullet list
    for h in hList:
        homingBulletList.remove(h)
...
def drawScene(surface,cur_pos,player,bList,delay,mb,peacefulAIList,mx,my,rx,ry,
              ...
              gameCountdown,countdownFont,countdownFontCol,wave,waveFontCol,waveFont,gunHUDimg,secHUDimg,
              txtList,imgList,damagedCount,healthIcon,defaultPlayerHealth,totalScore,score,multiplier,ratings,
              ratingRequirements,crosshairImg,invinCount):
    ...
    if player.health>0:
        ...
        for r in range(len(ratings)):
            if score*multiplier>=ratingRequirements[r]:
                rating = ratings[r]
    ...
    #------------------------------------------------HUD TEXT/IMAGES--------------------------------------------------
        crosshairImg = transform.rotate(crosshairImg,int(player.ang)+90)
        surface.blit(crosshairImg,(mx-crosshairImg.get_width()//2,my-crosshairImg.get_height()//2))
        mouse.set_visible(False)
        surface.blit(moneyFont.render("$ %s"%player.money,True,moneyFontCol),(5,70))
        surface.blit(moneyFont.render("%s X %s"%(score,multiplier),True,(234,231,34)),(5,130))
        totalScoreText = moneyFont.render("%s"%(totalScore),True,(217,125,16))
        surface.blit(totalScoreText,(rx-10-totalScoreText.get_width(),10))
        ratingText = moneyFont.render("%s"%(rating[1:]),True,(217,26,21))
        surface.blit(ratingText,(rx-10-ratingText.get_width(),100))
        letterFontType = font.SysFont('impact',100)
        letterText = letterFontType.render("%s"%(rating[0]),True,(217,26,21))
        surface.blit(letterText,(rx-10-letterText.get_width()-ratingText.get_width()-5,50))
        text=waveFont.render("WAVE %d"%wave.waveNum,True,waveFontCol)
        surface.blit(text,(screen.get_width()//2-text.get_width()//2,screen.get_height()-text.get_height()))
        ...
        surface.blit(gunHUDimg,(0,screen.get_height()-gunHUDimg.get_height())) #the weapon that the player has
        #print(secHUDimg)
        surface.blit(secHUDimg,(0,screen.get_height()-gunHUDimg.get_height()-secHUDimg.get_height())) #the melee weapon the player has
        #print((0,screen.get_height()-gunHUDimg.get_height()-secHUDimg.get_height()))
        
        if gameCountdown>0:
            text=countdownFont.render("%d"%(gameCountdown),True,countdownFontCol)
            surface.blit(text,(surface.get_width()//2-text.get_width()//2,surface.get_height()//2-text.get_height()//2-100))
        if invinCount > 0:
            new = surface.copy()
            new.set_alpha(50)
            screen.blit(new,(0,0))
        ...
        origSurface=screen.copy() #the screen that is blitted the frame b4 they die
        fadeOut(screen,origSurface)
        msg1 = moneyFont.render("SUPER-HITMAN DOWN",True,(217,26,21))
        msg2 = moneyFont.render("GAME OVER",True,(217,26,21))
        msg3 = moneyFont.render("FINAL SCORE: %s"%totalScore,True,(217,26,21))
        screen.blit(msg1,(rx//2-msg1.get_width()//2,ry//2-ry//4))
        screen.blit(msg2,(rx//2-msg2.get_width()//2,ry//2))
        screen.blit(msg3,(rx//2-msg3.get_width()//2,ry//2+ry//4))
...
def toggleSlowMo(screen,player,wave,AI,mx,my):
    ...
    """
    try:
        mousex=player.x-screen.get_width()//2+mx #mouse coord relative to the entire map
        mousey=player.y-screen.get_height()//2+my
        deltaY=AI.y-player.y
        deltaX=AI.x-player.x
        b=AI.y-(deltaY/deltaX)*AI.x
        A=deltaY
        B=-(deltaX)
        C=b*(-(B))
        dist=(abs(A*mousex+B*mousey+C))/sqrt(A**2+B**2)
    except ZeroDivisionError:
        dist=abs(player.x-mousex)
    #print(dist)
    if dist<200 and AI.blocked==False:
        global para
        para=True
...
def InsideGunStore(screen,gunStore,gunStoreCollide,player,secondary,meleeDelay,secWeap,meleeSprite,cx,cy,delay,ammoList,clipList,
                   weapons,lenPerks,reflectingBullets,reflectingCount,homingBullets,homingCount,playerTotalHealth):
    ...
    while inside:
        ...
        playerInStore=Player(in_x,in_y,0,player.pic1,player.pic1,player.money,player.health) #class for the player  inside store
        ...
        if gunStoreCollide.get_at((in_x,in_y))[:3]==(0,255,0) and k[K_e]:
            shop=Shop(screen,ammoList,clipList,lenPerks,reflectingBullets,reflectingCount,homingBullets,homingCount,totalPlayerHealth)
            shop.run(screen,ammoList,clipList,player,weapons,lenPerks,totalPlayerHealth)
        ...
        playerInStore.draw(screen,delay,mb,secondary,meleeDelay,secWeap,meleeSprite,mx,my,cx,cy,damagedCount=0)
        #---
...
class Shop:
    ...
    def __init__(self,screen,ammoList,clipList,lenPerks,
        ...
        self.aboveHealthScreen=Surface((self.healthScreen.get_width(),self.healthScreen.get_height())) #the image that is blitted above the healthScreen
        amounts=[25,50,100] #the amounts of health that you can buy
        costs=[totalPlayerHealth//4*3,totalPlayerHealth//2*3,totalPlayerHealth*3]
        for i in range(3):
            text1=font.SysFont("impact",75).render("%d%% of total health -> $ %d"%(amounts[i],costs[i]),True,(255,255,255))
            self.aboveHealthScreen.blit(text1,(20,self.aboveHealthScreen.get_height()//3*2//3*i+self.aboveHealthScreen.get_height()//3))
            self.healthBuyRects.append(Rect(screen.get_width()-buy.get_width()-10,
                                            int((self.aboveHealthScreen.get_height()//3*2//3*i+self.aboveHealthScreen.get_height()//3)/
                                                self.aboveHealthScreen.get_height()*screen.get_height()),
                                            buy.get_width()+10,buy.get_height()+10))

        self.aboveHealthScreen.set_colorkey((0,0,0))
    ...
    def checkEvents(self,screen,mx,my,lenPerks,totalPlayerHealth):
        ...
        for e in event.get():
                if e.type==VIDEORESIZE: #allows the screen to be resized
                    ...
                    cx,cy = resx//2,resy//2 #center coordinates, also half size of resolution
                    #print(e.dict['size'])
                if e.type==QUIT  or (e.type == KEYDOWN and e.key == K_ESCAPE): #only way to exit is to press the X button at the top of the screen
                    self.shopping=False
                ...
                if e.type==MOUSEBUTTONDOWN and e.button==1: #if mouse is left-clicked
                    ...
                    if self.page=="ammo":
                        ...
                        for rect in self.healthBuyRects:
                            if rect.collidepoint((mx,my)):
                                pos=self.healthBuyRects.index(rect) #which rect is pressed
                                if pos==0 and player.money>=totalPlayerHealth//4*3:
                                    self.ChaChing.play()
                                    player.money-=totalPlayerHealth//4*3
                                    player.health+=totalPlayerHealth//4
                                    if player.health>totalPlayerHealth:
                                        player.health=200
                                elif pos==1 and player.money>=totalPlayerHealth//2*3:
                                    self.ChaChing.play()
                                    player.money-=totalPlayerHealth//2*3
                                    player.health+=totalPlayerHealth//2
                                    if player.health>totalPlayerHealth:
                                        player.health=200
                                elif pos==2 and player.money>=totalPlayerHealth*3:
                                    self.ChaChing.play()
                                    player.money-=totalPlayerHealth*3
                        ...
                        for rect in self.perksBuyRects: #checks all of the buy buttons
                            if rect.collidepoint((mx,my)):
                                ...
                                if pos==0 and player.money>=self.perkCosts[pos]: #if they bought reflecting bullets
                                    ...
                                    player.money-=self.perkCosts[pos]
    ...
    def run(self,screen,ammoList,clipList,player,weapons,lenPerks,totalPlayerHealth):
        ...
        while self.shopping:
            ...
            self.checkEvents(screen,mx,my,lenPerks,totalPlayerHealth) #running loop          
            ...
            if self.page=="ammo" and self.ammoFlag==False:
                ...
                screen.blit(transform.scale(self.aboveHealthScreen,(screen.get_width()//4*3,screen.get_height())),(screen.get_width()//4,0))
                for rect in self.healthBuyRects:
                    draw.rect(screen,(0,255,0),rect)
                    screen.blit(self.buy,(rect[0]+5,rect[1]+5))
            ...
            if self.ammoFlag==True:
                ...
                screen.blit(transform.scale(self.aboveHealthScreen,(screen.get_width()//4*3,screen.get_height())),(self.healthPos,0))
            ...
            text=font.SysFont("impact",75).render("$ %d"%player.money,True,(0,255,0))
...
def game():
    ...
    global bSpeed
    global secondary
    global reflectingBullets
    global homingBullets
    global reflects
    global secDelay
    global grenade
    global grenade2
    ...
    global grenadeAng
    global grenadeDelay
    global peacefulAIList
    global shooterAIList
    global cur_x,cur_y
    global ammoList
    global clipList
    global coinList
    global collectedCoinList
    global droppedClipList
    global collectedClipList

    resx,resy = 1280,720 #resolution size
    cx,cy = resx//2,resy//2
    pic1,pic2 = weapon[0][0], weapon[0][1]
    reloadingSprites=reloadList[w]
    player = Player(cur_x,cur_y,0,pic1,pic2,money,playerHealth,reloadingSpeed,reloadList[w],ammoList[w],clipList[w],reloadingFrame,reloading,hasReloaded)
    wave=Wave() #initializing waves
    running = True
    while running:
        for e in event.get():
            ...
            if e.type==USEREVENT+1: #if a second has passed
                if wave.waveNum==0:
                    gameCountdown-=1
                    if gameCountdown==0:
                        #gameCountdown=0
                        wave.startGame()
                elif gameCountdown>0:
                    gameCountdown-=1
                    if gameCountdown==0:
                        enemyHealth=wave.newWave(enemyHealth)
                        #print(enemyHealth)
                #Updates music
                if int(curSong[1])==secondsPlayed: #if its done playing
                    curSong=choice(musicList) #play a new song
                    secondsPlayed=0
                    curSong[0].play()
                    #print("TRUE")
                else :
            ...
            if e.type==VIDEORESIZE: #allows the screen to be resized
                ...
                #print(e.dict['size'])
            if e.type==QUIT or (e.type == KEYDOWN and e.key == K_ESCAPE):
                running=False #resets game statitics
                gameCountdown = 10
                peacefulAIList = []
                shooterAIList = []
                coinList = []
                collectedCoinList = []
                ammoList = [20, 12, 50, 12]
                clipList = [20, 12, 50, 24]
                droppedClipList=[] #the list of ammo clips dropped by dead enemies
                collectedClipList=[] #the list of ammo clips collected by the player
                score = 0
                totalScore = 0
    ...
    #----------------------------------------------FUNCTIONS CONSTANTLY USED------------------------------------------------
        ...
        if secondary:
            ...
            elif secWeapon[0] == 'grenade' and grenade2 == False and grenadeIdle == False and exploded:
                secDelay,melSpr = result[0],result[1] #activates grenade sprites/damage
                throw = True
                if secDelay == 0:
                    exploded = False
                    throw = False
    ...
    #either the default throwing distance or shorter if the player wishes it to be so
        ...
        while len(peacefulAIList)<4: #spawns 4 random civilians in 4 random positions on the city map
        ...
        while len(shooterAIList)<wave.maxEnemies and wave.enemiesLeft-len(shooterAIList)>0: #makes shooters
            ai=makeShooterAI(screen,gameMap,collide,player,choice(weaponsAI),enemyHealth,AIspeed,AIammo)
        ...
        #print(wave.enemiesLeft,wave.waveNum)
        ...
        (totalScore,score,scoreMultiplier,imgList,invincibleCount) = erase(player,peacefulAIList,bulletList,coinList,deadList,dmgTip,secondary,secWeapon[0],
                                               ...
                                               greny,impactTime,collide,horizontal,wave,score,totalScore,scoreMultiplier,knifePts,
                                               bulletPts,homingPts,reflectPts,grenadePts,killDeduc,reflectingBulletList,imgList,
                                               texts,invincibleSec,invincibleCount)
        #print(gameCountdown)
        #erases bullets that already collided and civilians who've lost all health
        ...
        drawScene(onscreen,(player.x,player.y),player,bulletList,delay-1,mb,peacefulAIList,mx,my,resx,resy,viewDelay,bWid,bCol,
                  ...
                  meleeList[0][1][4],grenadeRot,grenadeIdle,explosion,explosionSpr,gameCountdown,countdownFont,countdownFontCol,
                  wave,waveFontCol,waveFont,HUDList[w],secHUDlist[n],texts,imgList,invincibleCount,healthImg,
                  playerHealth,totalScore,score,scoreMultiplier,letterGrades,ratingReq,crosshair,invincibleCount)
        #print(n)
        #draws all elements on screen (E.g. background, player, AIs, bullets)
        #print(grenadeHUD.get_width()//grenadeHUD.get_height()*secHUDimgHeight)
        ...
        if collide.get_at((player.x,player.y))[:3]==(0,0,255):
            ...
            if k[K_e]:
                ...
                InsideGunStore(screen,gunStore,gunStoreCollide,player,secondary,secDelay,secWeapon[0],melSpr,cx,cy,delay,
                               ammoList,clipList,weapons,lenPerks,reflectingBullets,reflectingCount,homingBullets,homingCount,totalPlayerHealth)
                curSong=choice(musicList) #play a new song
                secondsPlayed=0
                curSong[0].play()
                weapon=weapons[w]
        ...
        for e in event.get():
            ...
            if e.type==MOUSEBUTTONDOWN and e.button==1:
                if playRect.collidepoint((mpos)):
                    return('game')
                if controlsRect.collidepoint((mpos)):
