...
invincibleCount = 0 #counter to keep track of invincibility timer
...
crosshair = transform.scale(image.load('crosshair.png'),(100,100)) #new stuff
...
grenadeListN = 0 #number of default grenade sprite thrown
...
def makeHomingBullet(ang,wNum,shots,pos,damage,bLen):
    ...
    ((pos[0]+cos(ang)*35)+cos(ang+radians(-14+randang)) * 100-bLen,
     ...
     ang+radians(-14+randang)), #shotgun bullets
                (pos[0] + cos(ang+radians(-5+10*shots)) * 100-bLen, pos[1] -
        sin(ang)+radians(-5+10*shots) * 100-bLen, ang)] #dual pistol bullets
...
def drawScene(surface,cur_pos,player,bList,delay,mb,peacefulAIList,mx,my,rx,ry,
              ...
              ratingRequirements,crosshairImg,invinCount): #new stuff
    ...
    #------------------------------------------------HUD TEXT/IMAGES--------------------------------------------------
        crosshairImg = transform.rotate(crosshairImg,int(player.ang)+90) #new stuff
        surface.blit(crosshairImg,(mx-crosshairImg.get_width()//2,my-crosshairImg.get_height()//2)) #new stuff
        mouse.set_visible(False)
        surface.blit(moneyFont.render("$ %s"%player.money,True,moneyFontCol),(5,70)) #blits total money
        ...
        if invinCount > 0: #new stuff
            new = surface.copy()
            new.set_alpha(50)
            screen.blit(new,(0,0))
        else:
            screen.blit(surface,(0,0))
        #image.save(screen, 'help.png')
    else:
...
def InsideGunStore(screen,gunStore,gunStoreCollide,player,secondary,meleeDelay,secWeap,meleeSprite,cx,cy,delay,ammoList,clipList,
    ...
    while inside:
        ...
        mouse.set_visible(True) #new stuff
...
while running:
    for e in event.get():
        if e.type==MOUSEBUTTONDOWN:
            if e.button ==2:
                if n == 0 and grenadeListN == 0:
                    grenadeListN = 1
                elif n == 0 and grenadeListN == 1:
                    n = 1
                    grenadeListN = 0
                elif n == 1:
                    n = 0
...
#either the default throwing distance or shorter if the player wishes it to be so
    ...
    drawScene(onscreen,(player.x,player.y),player,bulletList,delay-1,mb,peacefulAIList,mx,my,resx,resy,viewDelay,bWid,bCol,
              ...
              meleeList[grenadeListN][1][4],grenadeRot,grenadeIdle,explosion,explosionSpr,gameCountdown,countdownFont,countdownFontCol,
